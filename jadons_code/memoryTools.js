module.exports = {
    ...
    cleanConstruct: function(targetRoom) {
        let allStructures = targetRoom.find(FIND_STRUCTURES);
        for (let structure in Memory.structures) {
            let targetStructure = Game.getObjectById(structure);
            if (!allStructures.includes(targetStructure)) {
